 ...
 */
@Service
public class FlashcardPackageServiceImpl extends
    ServiceImpl<FlashcardPackageMapper, FlashcardPackage> implements FlashcardPackageService {
  ...
  public Boolean updateCardPackage(PackageInfoDto packageInfoDto, Long userId) {
    ...
    // 分标签的新增和删除
    Map<String, String> labelMap = packageInfoDto.getLabelInfos().stream()
    ...
    if (!CollectionUtils.isEmpty(labelMap.keySet())) {
      ...
      Map<String, String> labelMapFromDb = labelFromDb.stream()
      ...
      labelMap.forEach((k, v) -> {
        if (!labelMapFromDb.containsKey(k)) {
          ...
          // 将卡包关联的标签移除
          labelRefQueryWrapper.eq("ref_uuid", packageInfoDto.getUuid()).eq("label_uuid", k).or();
        }
      });
