 ...
 */
@Service
public class DialogueSceneServiceImpl extends ServiceImpl<DialogueSceneMapper, DialogueScene>
    implements DialogueSceneService {
  ...
  public Boolean updateDialogueScene(SceneData sceneData) {
    ...
    List<DialogueData> dialoguesOfScene =
        dialogueSceneMapper.getDialoguesOfScene(sceneData.getUuid());
    ...
    // 找到新增的数据
    dialogueDataList.stream().filter(e -> org.apache.commons.lang3.StringUtils.isBlank(e.getUuid()))
        .forEach(e -> {
    ...
    if (!CollectionUtils.isEmpty(deleteDialogues)) {
      ...
      List<String> dialoguesUUIDList =
          deleteDialogues.stream().map(Dialogue::getUuid).collect(Collectors.toList());
