...
public class FlashcardServiceImpl extends ServiceImpl<FlashcardMapper, Flashcard>
  ...
  public AjaxResult batchAddCard(BatchAddCardDto batchAddCardDto) {
    if (Objects.equals(batchAddCardDto.getCardType(), CardTypeEnum.EXPRESSION.getValue())) {
      // 获取表达内容，构造卡片
      List<ExpressionData> expressionDataList =
          expressionService.getExpressionListByUUIds(batchAddCardDto.getUuidList());
      ...
      for (ExpressionData expression : expressionDataList) {
        ...
        Flashcard flashcard = buildFlashCard(batchAddCardDto.getUserId(), expression.getContent(),
            back, batchAddCardDto.getCardType(), expression.getUuid());
  ...
  public AjaxResult batchAddCardByWordCollection(PackageCollectionData packageCollectionData) {
    ...
    if (packageCollectionData.getCardType() == 1) {
      // 单词类型卡片
      List<Word> wordsOfCollection =
          wordCollectionService.getWordsOfCollection(packageCollectionData.getCollectionUUID());
      ...
      operateWordInCollection
          .setPackageUUIdList(Collections.singletonList(packageCollectionData.getPackageUUID()));
  ...
  public AjaxResult addCardsToPackage(PackageCardInfo packageCardInfo) {
    ...
    if (!CollectionUtils.isEmpty(packageCardRefList)) {
      Set<String> existCardUUIdList =
          packageCardRefList.stream().map(PackageCardRef::getCardUuid).collect(Collectors.toSet());
  ...
  private Flashcard buildFlashCard(Long userId, String front, String back, Integer cardType,
      String sourceUUId) {
    Flashcard flashcard = new Flashcard();
