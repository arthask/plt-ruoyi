...
public class FastAutoGeneratorTest extends BaseGeneratorTest {
  ...
  private static final DataSourceConfig.Builder DATA_SOURCE_CONFIG = new DataSourceConfig.Builder(
      "jdbc:mysql://localhost:3306/ry_vue?serverTimezone=Asia/Shanghai", "root", "202312");
  ...
  public static void main(String[] args) {
    ...
    // 数据库表的设置
    List<String> listTable = Arrays.asList("dialogue_scene"); // 设置需要自动代码生成的表名
    ...
    FastAutoGenerator.create(DATA_SOURCE_CONFIG)
        ...
        .globalConfig((scanner, builder) -> builder
            .outputDir(System.getProperty("user.dir") + "/pltool/src/main/java").author("author")
            // .enableKotlin()
            // .enableSwagger()
            .dateType(DateType.TIME_PACK).commentDate("yyyy-MM-dd").disableOpenDir())
        // 包配置
        ...
        .strategyConfig(builder -> {
          builder.enableCapitalMode() // 开启大写命名
              ...
              .enableLombok().versionColumnName("version") // 乐观锁字段名(数据库)
              ...
              // .enableFileOverride()
              // 4.4、Mapper策略配置
              .mapperBuilder().enableFileOverride().superClass(BaseMapper.class) // 设置父类
        ...
        // 5、模板
        .templateEngine(new VelocityTemplateEngine()).templateEngine(new FreemarkerTemplateEngine())
        /*
         * 模板引擎配置，默认 Velocity 可选模板引擎 Beetl 或 Freemarker 或 Enjoy .templateEngine(new
